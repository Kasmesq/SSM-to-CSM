...
void Graph::BuildReverseIndex() {
    reverse_index_ = new ui[vertices_count_]; // store vertex index
    reverse_index_offsets_= new ui[labels_count_ + 1];
    reverse_index_offsets_[0] = 0; //mark the starting positions of labels
...
//build reverse index for quick lookup of vertices by their labels
#if OPTIMIZED_LABELED_GRAPH == 1
void Graph::BuildNLF() {
    nlf_ = new sparse_hash_map<uint32_t, uint32_t>[vertices_count_]; // store count of each label in neighbors of each vertex
    for (ui i = 0; i < vertices_count_; ++i) {
        ui count;
...
// build Neighbor Label Frequency (NLF) for each vertex
...
// build label offset table ofr the graph.
// sort neighbors of each vertex by label and compute offsets for each lael
#endif
...
// load graph from memory, given vertex and edge lists
// data structure = {vertex labels, offsets, neighbors, edge labels}
// build various indices for efficient graph operations
...
//read graph from file in specific format
// file to contain vertices and edges, each with specific labels
// fills graph's data structures
// builds indices
// sort neighbors
...
//print metadata (# of vertices, edges, labels, max degrees, max label freq )
// |V|: 403394, |E|: 2443408, |Σ|: 7
// Max Degree: 2752, Max Label Frequency: 67689
...
// build core table for graph : k-core decomposition (use graphoperations::getkcore()) := compute core values
...
void Graph::loadGraphFromFileCompressed(const std::string &degree_path, const std::string &edge_path,
    ...
    if (is_edge_labeled) {
        std::ifstream edge_label_file(edge_label_path, std::ios::binary); //label
...
//compressed graph from binary files
//read degree ,adj, label, edge label files
//initialize graph's data structure
...
//store compressed graph to binary files
// wrtie degree, adj, label, edge label
...
// build edge index (hash map vertex labels to list of edges connecting vertives with those labels.
// ==> facilitates efficient edge lookups
...
//laod compressed graph from specific dir
...
//store compressed graph to specific dir
...
//read graph from file without assuming any metadata
//initialize vertex and edge, handling vertex labels and edge labels if they exist
//build index and convert graph to compressed sparse row format
